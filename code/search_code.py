...
rest_url = "https://api.github.com/search/code?q=\"chat.openai.com/share\""
fetch_rest_time = 60 / config.code_search_rate_limit_per_min
...
def get_all_pages(query):
    ...
    while current_amount < total_count and page <= 10:
...
results = []
try:
    for langugae in config.languages_top_50:
        print(f"Searching language:  {langugae}")
        items = get_all_pages(rest_url + f" language:{langugae}")
        results += items
        print(f"language {langugae}: {len(items)}")
        time.sleep(fetch_rest_time)
except Exception as e:
    print(e)
finally:
    with open(f"./search_results/code_search_results.json", encoding='utf-8', mode='w') as f:
        json.dump(results, f, indent=4)
    print(f"Type: code; SearchResultCount:{len(results)}")